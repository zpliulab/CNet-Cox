...
## data set
# GSE42568  GSE10780  GSE5764

setwd(paste(path, 'BRCA/Bulk_RNA-seq_data/GEO_noclin/GSE10780', sep=''))
...
# load data ---------------------------------------------------------------
# Data1 = read.table("GSE10780_outcome_scale_IDC.txt", header = T, check.names = FALSE)
...
# View(genedata2[,1:10])
write.table(genedata2,"Bulk_RNA-seq_data/GEO_noclin/GSE10780/GSE10780_6_ori.txt",quote=F,sep="\t")


# ????PRS??ϵ??,??ȡ??Ҫ??ϵ?? ----------------------------------------------------------------
phi_coef <- read.csv("TCGA_NEW/UniMutVariate_markergene.csv", header = T, sep=',')
genedata3 <- read.table("Bulk_RNA-seq_data/GEO_noclin/GSE10780/GSE10780_6_ori.txt", header = T, check.names = FALSE)
# View(genedata3[,1:10])
...
phi1[1,1]
write.table(phi1, file = "Bulk_RNA-seq_data/GEO_noclin/GSE10780/prs_GSE10780_NEW.txt", quote=F,sep="\t")


# ??????ͼ --------------------------------------------------------------------

pred_log = read.table("Bulk_RNA-seq_data/GEO_noclin/GSE10780/prs_GSE10780_NEW.txt", header = T, check.names = FALSE)
...
# pdf(file = "Box_GSE10780_NEW_color.pdf",width = 3,height = 4)
# plot
# dev.off()
