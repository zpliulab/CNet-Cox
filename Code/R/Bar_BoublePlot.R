...
p2 = gg.gap(plot = p1,
            segments = c(80, 500),
            tick_width = 20,
            rel_heights = c(0.25, 0, 0.1),    # Set the width of the three parts separated
            ylim = c(0, 550)
)
...
library(ggplot2)
library(ggpubr)
# Modify the horizontal axis order
p7 <- ggplot(datat, aes(x = factor(Status, levels = unique(Status)), y = TCF7, color = Status)) +
  ...
  geom_line(aes(group = Patient), 
            color = 'grey40', lwd = 0.5, position = position_dodge(0.5))+    #Add a connection
...
## The key step is to smooth each line.
...
## with the color Monocle3, looks a bit more interesting
