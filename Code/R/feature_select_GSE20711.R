...
## data set
# GSE42568  GSE20711  GSE5764 GSE10780

setwd(paste(path, 'BRCA/Bulk_RNA-seq_data/GEO_noclin/GSE20711', sep=''))
...
# load data ---------------------------------------------------------------
# Data1 = read.table("GSE20711_scale_outcome_os.txt", header = T, check.names = FALSE)
# View(Data1[,1:10])
Data1 = read.table("GSE20711_scale_outcome_rfs.txt", header = T, check.names = FALSE)
...
genedata2 <- rbind(Data1[2,],genedata1)
# View(genedata2[,1:10])
write.table(genedata2,"Bulk_RNA-seq_data/GEO_noclin/GSE20711/GSE20711_6_ori.txt",quote=F,sep="\t")


# ????PRS??ϵ??,??ȡ??Ҫ??ϵ?? ----------------------------------------------------------------
phi_coef <- read.csv("TCGA_NEW/UniMutVariate_markergene.csv", header = T, sep=',')
genedata3 <- read.table("Bulk_RNA-seq_data/GEO_noclin/GSE20711/GSE20711_6_ori.txt", header = T, check.names = FALSE)
# View(genedata3[,1:10])
...
pred_log[which(pred_log[,2] == "0"),2]  <- c("Normal")
dim(pred_log)
...
# pdf(file = "Box_GSE20711_NEW_color.pdf",width = 3,height = 4)
# plot
# dev.off()
