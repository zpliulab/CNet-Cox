function [lambda_opt, alpha_opt, r] = cvCox(X,ytime,y, L,alpha,lambda,nfold)
...
R_matrix1 = RiskMatrix(ytimenot1);
...
fnw=['R_matrix',int2str(nfold),' = RiskMatrix(ytimenot',int2str(nfold),');'];
eval(fnw);
...
eval(fnw);
% calculate the Risk matrix under the selected fold ytime
fnw=['R_matrix',int2str(i),' = RiskMatrix(ytimenot',int2str(i),');'];
eval(fnw);
